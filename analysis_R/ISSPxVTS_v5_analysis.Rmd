...
### 2. Choice trials
#### Analyze with RT/ACC with 2 swProb x trialType_sw 
```{r}
gpCondM2 <- tibble()
gpCondM2 <- gpData %>% 
...
### 4. Choice trials
#### Analyze VSR  with 2 runId_half x 2 swProb 
```{r}
...
```{r Figure7}
gp_choice2 %>%
  ggplot(aes(x=swProb, y = gpmeanVSR, fill = runId_half)) + 
  geom_col(position = dodge) + 
  geom_errorbar(aes(ymax = gpmeanVSR + (1*SE_vsr), ymin =gpmeanVSR -(1*SE_vsr)), position = dodge, width=.25) +
  ylab("mean VSR") + 
  coord_cartesian(ylim = c(30,40)) +
...
data.anova.mean <- aggregate(tt$trialType_int,
                             by = list(sub = tt$sbjId,
                                       ...
                                       p = tt$swProb),
                             FUN = 'mean', na.rm = TRUE)
bf <- anovaBF(x ~ u * p + sub, data = data.anova.mean, 
             whichRandom="sub")
plot(bf)
...
anova <- aov(x ~  p + Error(sub/(p)),
                    data = data.anova.mean)


bf <- anovaBF(x ~ p + sub, data = data.anova.mean, 
             whichRandom="sub")
plot(bf)
