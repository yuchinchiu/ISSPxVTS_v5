...
gpData[which(gpData$phase=="hybrid" & gpData$runId==3), "runId2"]="first2"
...
catVblList <- c("phase","sbjId","runId", "swProb", "task", "trialType_sw", "runId_half","runId2")
...
z = ezANOVA(data = gpCondM5
            ...
            , detailed = TRUE)
print(z)
z_table <- apa.ezANOVA.table(z, filename="VSR_2unevenHalfx2SwProb.doc")
...
c   <- 0
for(f in factorCol){
  tt[[f]] <- as.character(tt[[f]])
  c <- c+1
  arg[c] <- paste("tt$",colNames[f],sep = "")
}

cmd <- paste0("paste0(", paste(arg,collapse = ","),")") 
tt$name <-eval(parse(text=cmd))

tt2 <- tt %>% 
  select(sbjId, meanRT, name)
summary <- spread(tt2,key = name,value = meanRT)
...
tt2 <- tt %>% 
  select(sbjId, meanRT, name)
tt2 <- spread(tt2,key = name,value = meanRT)



summary$ISSP_choice <- (tt2$`sw25%switch`-tt2$`sw25%repeat`)-(tt2$`sw75%switch`-tt2$`sw75%repeat`)
...
tt2 <- spread(tt,key = swProb,value = meanVSR)

summary$VSReffect <- tt2$`sw75%`-tt2$`sw25%`
