...
```{r}
z = ezANOVA(data = gpCondM2[gpCondM2$trialType=="cued",]
            , dv = meanRT
            , wid = sbjId
...
df <- gpCondM3 %>% select(sbjId, swProb, tskRatio)
colnames(df)[colnames(df)=="tskRatio"] <- "M"
LnMSE_tskRatio <- getWSSE(df)


tskBias <- gpCondM3 %>%
  group_by(swProb) %>%
  summarise(gpmeanTskRatio = mean(tskRatio), SE_tskRatio = LnMSE_tskRatio)

tskRatio_25 = tskBias[1,"gpmeanTskRatio"]*100
tskRatio_75 = tskBias[2,"gpmeanTskRatio"]*100

print(paste("tskRatio_25% swProb =", sprintf("%.2f", tskRatio_25), "% of task 1"))
print(paste("tskRatio_75% swProb =", sprintf("%.2f", tskRatio_75), "% of task 1"))

t.test(formula = tskRatio~swProb, data = gpCondM3)

tskBias  %>%
  ggplot(aes(x=swProb, y = gpmeanTskRatio*100)) + 
  geom_col(position = dodge) + 
  geom_errorbar(aes(ymax = gpmeanTskRatio*100 + (1*SE_tskRatio*100), ymin =gpmeanTskRatio*100 -(1*SE_tskRatio*100)), 
                position = dodge, width=.25) + 
  ylab("mean Switch Rate(%)") + 
...
```{r}
gp_VSR <- aggregate(gpData$trialType_int,
                             by = list(sub = gpData$sbjId,
                                       run = gpData$runId_half,
                                       swProb = gpData$swProb),
                             FUN = 'mean', na.rm=TRUE)

#anova <- aov(x~ run * swProb  + Error(sub/run*swProb), 
#             data = gp_VSR)

z = ezANOVA(data = gp_VSR
            , dv = x
            , wid = sub
            , within = .(run, swProb)
            , within_full = .(run, swProb)
            , type = 3
            , detailed = TRUE)
print(z)
...
gp_VSR.test <- gp_VSR %>%
  filter(sub %in% sample(gp_VSR$sub,20))

bf.test <- anovaBF(x~ run * swProb  + sub, 
              data = gp_VSR.test, 
              whichRandom="sub")
